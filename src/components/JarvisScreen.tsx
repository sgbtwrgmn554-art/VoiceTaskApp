...
type JarvisState = 'idle' | 'loading' | 'speaking' | 'listening' | 'thinking';
...
export default function JarvisScreen({ tasks, goals, habits, aiLanguage = 'hebrew', aiStyle = 'brief', accentColor, onClose }: Props) {
  ...
  const startListening = useCallback(() => {
    ...
    rec.onresult = (e: any) => {
      ...
      if (text.trim()) sendQuestion(text.trim());
    };
    ...
    try {
      const res = await fetch('/api/chat', {
        ...
        body: JSON.stringify({
          ...
          tasks, aiLanguage, aiStyle,
        }),
      });
      const data = await res.json();
      speakAndListen(data.assistantText || 'מצטער, לא הצלחתי לקבל תשובה.');
    } catch {
      speakAndListen('מצטערת, יש בעיה בחיבור.');
    }
  }, [addMsg, tasks, aiLanguage, aiStyle, speakAndListen]);
  ...
  const pulseColor = state === 'listening' ? '#ef4444'
    ...
    <div className="fixed inset-0 z-50 flex flex-col" style={{ background: '#050505' }}>
      ...
      <div className="flex-shrink-0 px-6 pb-8 pt-4 flex items-center gap-4">
        {!started ? (
          ...
          <>
            <button
              onClick={() => { stop(); try { recognitionRef.current?.stop(); } catch {} setState('idle'); }}
            ...
            <button
              ...
              style={{
                background: state === 'listening' ? '#ef444422' : accentColor + '18',
                border: `2px solid ${state === 'listening' ? '#ef4444' : accentColor}`,
                color: state === 'listening' ? '#ef4444' : accentColor,
              }}
            >
              <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor">
                <path d="M12 1a4 4 0 014 4v6a4 4 0 01-8 0V5a4 4 0 014-4zm-1 17.93A8.001 8.001 0 014 11H2a10 10 0 0019.95 1H20a8 8 0 01-7 7.93V23h-2v-4.07z"/>
              </svg>
              {state === 'listening' ? 'מאזין...' : 'שאל שאלה'}
            </button>
          </>
        )}
      </div>
